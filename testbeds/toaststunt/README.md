...
- `./setup.sh` builds `~/src/toaststunt` out of tree into `.build/` (override with
  `TOASTSTUNT_SRC`; installs missing brew deps) and prepares `.run/base.db` from a copy of
  `~/src/toastcore/toastcore.db` (`TOASTCORE_DB`). Idempotent; `--reset` re-prepares.
